...
//public class UnitCombatOld
//{
//    public static int COST_TO_ATTACK = 60;

//    private readonly GameManager unitManager;
//    private readonly TileManager tileManager;
//    //private readonly UnitMovementOld unitMovement;
...
//    public UnitCombatOld(  KeyboardControls keyboardControls, 
//                        GameManager unitManager,
//                        LineRenderer lineRenderer,
//                        TileManager tileManager,
//                        //UnitMovementOld unitMovement)
//    {
...
//    {
//        AttackTargetedUnit(unitManager.GetPositionByUnit(unitManager.GetSelectedUnit()));
//    }
...
//        unitManager.Enqueue(
//            unitMovement.MoveToDestinationRoutine(unitManager.GetSelectedUnit(), new Queue<Node>(new Node[] { new Node(attackFromCoordinates) }))
//            );
...
//        unitManager.Enqueue(
//            unitMovement.MoveToDestinationRoutine(unitManager.GetSelectedUnit(), new Queue<Node>(new Node[] { new Node(unitOriginalPosition) }))
//            );

//        unitManager.GetSelectedUnit().UseActionPoints(COST_TO_ATTACK);
...
//                                    .Where(x => unitManager.GetUnitByPosition(x).Faction != unit.Faction
//                                                && IsTargetAttackable(source, x, rangeTiles, unit.ActionPointsLeft, out _, out _))
//                                    .Select(x => unitManager.GetUnitByPosition(x)));
//    }

//    public HashSet<Unit> GetReceivableAttacks(Vector3Int tilePosition, Unit receivingUnit)
//    {
...
//            if (unit.Faction == receivingUnit.Faction)
//            {
//                continue;
//            }
...
//        if (unitManager.GetSelectedUnit().Faction == Faction.Friendly && unitManager.PositionsOfUnits.ContainsKey(gridPosition)) 
//        {
...
//        rangeTiles = GetRangedTiles(unitManager.GetPositionByUnit(unitManager.GetSelectedUnit()), unitManager.GetSelectedUnit().Range);
...
//        bool hasSightToTarget = lineRaytracer.Trace(offensiveUnitPosition, targetPosition, GameManager.UNIT_GRID_OFFSET, tileManager);
...
//                canPeekAndShoot = lineRaytracer.Trace(peekCoordinates, targetPosition, GameManager.UNIT_GRID_OFFSET, tileManager);
...
//    {
//        return IsTargetAttackable(unitManager.GetPositionByUnit(unitManager.GetSelectedUnit()),
//                                  targetPosition,
//                                  rangeTiles,
//                                  unitManager.GetSelectedUnit().ActionPointsLeft,
//                                  out sourcePosition,
//                                  out tilesHit);
//    }
...
//        lineRenderer.SetPosition(0, tileManager.Ground.CellToWorld(sourcePosition) + GameManager.UNIT_WORLD_OFFSET);
//        lineRenderer.SetPosition(1, tileManager.Ground.CellToWorld(tilesOverlapping[tilesOverlapping.Length - 1]) + GameManager.UNIT_WORLD_OFFSET);
...
//    {
//        int effectiveDamage = Math.Min(unit.Defence - unitManager.GetSelectedUnit().Attack, 0);
