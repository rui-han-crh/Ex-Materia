...
public partial class GameManagerOld : MonoBehaviour
{
    ...
    public GameMapOld CurrentMap => currentMap;
    ...
    private void Start()
    {
        ...
        currentMap = new GameMapOld(unitPositionMap, fullCoverPositions, halfCoverPositions, groundTileData.Keys, groundTileData);
        Debug.Log($"Initialised: {currentMap}");

        InitialiseHealthBar();
        UnitQueueManagerOld.Instance.InitialiseQueue(unitGameObjects, currentMap.AllUnits.ToDictionary(unit => unit.Name, unit => unit));
    ...
    private void Update()
    {
        ...
        switch (gameState)
        {
            ...
            case GameState.TurnEnded:
                ...
                UnitQueueManagerOld.Instance.UpdateUnitQueue(currentMap.AllUnits.ToDictionary(unit => unit.Name, unit => unit));
        ...
        if (IsRoutineQueueDormant())
        {
            ...
            else if (!UnitQueueManagerOld.Instance.IsPlayingAnimation && !autoPlayQueued && gameState == GameState.OpponentTurn)
            {
                AutoPlay();
            }
        }
    ...
    public void SelectCommand(Command command)
    {
        ...
        switch (command)
        {
            case Command.Movement:
                ...
                request = new MovementRequestOld(CurrentMap, CurrentUnitPosition, pathPositionsLastDrawn.ToArray(), lastActionCost);
                break;
            ...
            case Command.Attack:
                ...
                request = new AttackRequestOld(CurrentMap,
            ...
            case Command.Wait:
                request = new WaitRequestOld(CurrentMap, CurrentUnitPosition, timeToWait);
                break;

            case Command.Overwatch:
                request = new OverwatchRequestOld(CurrentMap, CurrentUnitPosition);
                break;
    ...
    public void ParseRequest(MapActionRequestOld request)
    {
        int cost;
        switch (request.ActionType)
        {
            case MapActionType.Movement:
                MovementRequestOld movementRequest = (MovementRequestOld)request;
                ...
                IEnumerable<AttackRequestOld> positionsAttacked = currentMap.OverwatchersCanAttackAny(movementRequest.Path);
                positionsAttacked.OrderBy(x => Vector3.Magnitude(CurrentUnitPosition - x.TargetPosition));
                int startingIndex = 0;

                foreach (AttackRequestOld attackRequestByOverwatch in positionsAttacked)
                {
            ...
            case MapActionType.Attack:
                AttackRequestOld attackRequest = (AttackRequestOld)request;
            ...
            case MapActionType.Wait:
                WaitRequestOld waitRequest = (WaitRequestOld)request;
                routineQueue.Enqueue(CurrentUnitRecoverAP(waitRequest));
                break;

            case MapActionType.Overwatch:
                OverwatchRequestOld overwatchRequest = (OverwatchRequestOld)request;
