...
namespace DirectedGraphTests {
    public class DirectGraphInitialisations
    {
        ...
        [Test]
        public void GraphRemoveWithoutDisconnecting()
        {
            IDirectedGraph<Vector3Int> graph = new DirectedGraph<Vector3Int>();

            graph.Add(new Vector3Int(3, 3, 3));
            graph.Add(new Vector3Int(4, 4, 5));
            graph.Connect(new Vector3Int(3, 3, 3), new Vector3Int(4, 4, 5));
            
            graph.Remove(new Vector3Int(4, 4, 5));

            Assert.IsFalse(graph.IsConnected(new Vector3Int(3, 3, 3), new Vector3Int(4, 4, 5)));
            Assert.AreEqual(0, graph.GetConnected(new Vector3Int(3, 3, 3)).Count());

            Assert.IsFalse(graph.Contains(Vector3Int.one));
            Assert.IsTrue(graph.Contains(new Vector3Int(3, 3, 3)));
            Assert.IsFalse(graph.Contains(new Vector3Int(4, 4, 5)));
        }
